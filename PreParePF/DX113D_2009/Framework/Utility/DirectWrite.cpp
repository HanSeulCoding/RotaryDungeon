...
void DirectWrite::Add(string key, wstring font, Float3 color, float size, DWRITE_FONT_WEIGHT weight, DWRITE_FONT_STYLE style, DWRITE_FONT_STRETCH stretch)
{
...
void DirectWrite::RenderText(wstring text, string key, Float2 pos, Float2 size)
{
	if (fonts.count(key) == 0)
		return;
	...
	context->DrawTextW(text.c_str(), (UINT)text.size(), fonts[key]->format, &rectF, fonts[key]->brush);
}
